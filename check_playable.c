...
*/
void search_nextway(t_data *d, t_object current, int **visited, t_list **list, ssize_t i)
{
    ssize_t x;
    ssize_t y;
    int     searchlist[4][2];
...
int search_root(t_data *d, int **visited, ssize_t arg_x, ssize_t arg_y)
{
    ...
    while (list)
    {
        ...
        i = 0;
        while(i < 4)
            search_nextway(d, current, visited, &list, i++);
    }
    while (list)
        get_from_list(&list);
    return (ret);
		...
		while(y < d->map->row)
		{
			if(d->map->map[y][x] == ITEM)
                if(is_reachable(d, x, y) != 1)
                    free_and_exit("Error\n  You must check the map for unreachable items\n", d);
            y++;
        }
        x++;
    }
